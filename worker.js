...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/chat'            && method === 'POST') return handleChat(request, env, ctx);
      ...
      if (url.pathname === '/ask-question'        && method === 'POST') return handleAskQuestion(request, env, ctx);
      if (url.pathname === '/get-answer'          && method === 'GET')  return handleGetAnswer(url, request, env, ctx);
      if (url.pathname === '/admin/trigger-batch' && method === 'POST') return handleTriggerBatch(request, env);
...
async function handleChat(request, env, ctx) {
  ...
  const { caseId, message, conversationHistory = [], askedIntents = [], personality = 'neutral' } = body;
  ...
  if (!intent) {
    ...
    if (env.SUPABASE_URL && env.SUPABASE_ANON_KEY) {
      // Check Worker KV cache first (instant, free)
      const kvKey = workerAnswerKey(caseId, message);
      if (env.CASES_KV) {
        const kvHit = await env.CASES_KV.get(kvKey);
        if (kvHit) {
          return json({ reply: kvHit, intentId: null, type: 'cached', isDangerous: false, score: 0, source: 'worker_kv' });
        }
      }
...
const INTENT_PATTERNS = [
  ...
  { id:'sr_fast_breathing',  keywords:['fast breathing','rapid breathing','breathing fast','tachypnoea','respiratory rate','breathing quickly','breathing too fast'],
    phrases:['is he breathing fast','is she breathing fast','any fast breathing','breathing fast','rapid breathing'] },
  { id:'sr_chest_indrawing', keywords:['chest indrawing','chest drawing','in-drawing','subcostal','intercostal','retraction','sinking chest'],
    phrases:['any chest indrawing','chest wall indrawing','lower chest wall','chest sinking'] },
  { id:'sr_nasal_flaring',   keywords:['nasal flaring','nose flaring','nostril flaring','flaring nostrils'],
    phrases:['any nasal flaring','nostrils flaring','nose flaring'] },
  { id:'sr_grunting',        keywords:['grunting','grunt','groaning','moaning'],
    phrases:['any grunting','is he grunting','grunting sound'] },
  { id:'sr_cyanosis',        keywords:['blue','cyanosis','cyanosed','bluish','lips blue','finger blue','central cyanosis'],
    phrases:['lips turned blue','any bluish','any cyanosis','blue lips','blue fingers','finger tips blue'] },
  { id:'sr_apnea',           keywords:['apnea','apnoea','pause breathing','stop breathing','breath holding','cessation of breathing'],
    phrases:['any apnea','pause in breathing','stop breathing','breathing pauses'] },
  { id:'sr_noisy_breathing', keywords:['noisy breathing','stridor','stertor','noise breathing','breathing sound'],
    phrases:['any noisy breathing','noisy when breathing','breathing sounds'] },
  { id:'smoke_exposure',     keywords:['smoke','smoking','cigarette','firewood','kerosene','indoor smoke','secondhand'],
    phrases:['anyone smoke','smoke in the house','firewood cooking','kerosene stove','indoor air pollution'] },
  { id:'tb_contact',         keywords:['tuberculosis','tb','contact tb','tb patient','someone with tb','coughing contact'],
    phrases:['any tb contact','tuberculosis contact','contact with tb','anyone with tuberculosis'] },
  { id:'aspiration_history', keywords:['aspiration','choke','choking','swallow','foreign body','peanut','small object'],
    phrases:['any choking','choking episode','swallow something','foreign body','aspiration history'] },
  { id:'sr_oral_thrush',     keywords:['thrush','oral thrush','white patches mouth','candida','mouth sores'],
...
//  Plugs into handleChat as a silent fallback layer.
//  When the rule engine has no intent match, we:
//    1. Check Supabase reply_bank for a verified cached answer
//    2. If found → return it (student gets real answer)
//    3. If not   → queue the question silently + return deflection
//  Students never wait. The batch loop fills the reply_bank
//  4× per day via pg_cron.
// ══════════════════════════════════════════════════════════════

/**
 * Deterministic KV key — must match normalizeQuestion() in
 * ask-question/index.ts and get-answer/index.ts exactly.
 * lowercase → trim → strip trailing punctuation → collapse spaces
 * KV keys can't have spaces → URL-encode.
 */
function workerAnswerKey(caseId, question) {
  const norm = question
    .toLowerCase()
    .trim()
    .replace(/[?.,!;:]+$/, '')
    .replace(/\s+/g, ' ');
  return `llm_answer:${caseId}:${encodeURIComponent(norm)}`.substring(0, 512);
}

/** Call a Supabase edge function */
async function supabaseFetch(fnPath, options = {}, env, role = 'anon') {
  const key = role === 'service'
    ? env.SUPABASE_SERVICE_ROLE_KEY
    : env.SUPABASE_ANON_KEY;
  return fetch(`${env.SUPABASE_URL}/functions/v1${fnPath}`, {
    ...options,
    headers: {
      'Content-Type': 'application/json',
...
/**
 * POST /ask-question
 * Thin proxy — checks Worker KV first, then Supabase.
 * Used by frontend to directly queue questions.
 */
async function handleAskQuestion(request, env, ctx) {
  if (!env.SUPABASE_URL) return err('Supabase not configured', 503);
  const body = await request.json().catch(() => null);
  if (!body?.caseId || !body?.question) return err('caseId and question required');

  const { caseId, question, caseContext = {}, personality = 'neutral' } = body;

  // Layer 1: Worker KV cache
  const kvKey = workerAnswerKey(caseId, question);
  if (env.CASES_KV) {
    const hit = await env.CASES_KV.get(kvKey);
    if (hit) return json({ status: 'cached', answer: hit, source: 'worker_kv' });
  }
  ...
  if (!resp.ok) return json({ status: 'queued', fallback: "I'm not sure I understand. Could you rephrase?" });

  const data = await resp.json();

  // Populate Worker KV if Supabase returned a cached answer
  if (data.status === 'cached' && data.answer && env.CASES_KV) {
    ctx.waitUntil(env.CASES_KV.put(kvKey, data.answer, { expirationTtl: 60 * 60 * 24 * 7 }));
  }
...
/**
 * GET /get-answer?caseId=...&question=...
 * Three-layer cache: CF CDN → Worker KV → Supabase reply_bank
 */
async function handleGetAnswer(url, request, env, ctx) {
  if (!env.SUPABASE_URL) return json({ found: false });
  const caseId   = url.searchParams.get('caseId');
  const question = url.searchParams.get('question');
  if (!caseId || !question) return err('caseId and question required');

  // Layer 1: Cloudflare CDN cache
  ...
  if (env.CASES_KV) {
    const hit = await env.CASES_KV.get(kvKey);
    if (hit) {
      const resp = json({ found: true, answer: hit, source: 'worker_kv' });
      ctx.waitUntil(cfCache.put(request, resp.clone()));
      return resp;
    }
  }

  // Layer 3: Supabase
  const sbResp = await supabaseFetch(
    `/get-answer?caseId=${encodeURIComponent(caseId)}&question=${encodeURIComponent(question)}`,
    { method: 'GET' }, env, 'anon',
  );
  if (!sbResp.ok) return json({ found: false });
  const data = await sbResp.json();

  if (data.found && data.answer) {
    ctx.waitUntil((async () => {
      if (env.CASES_KV) await env.CASES_KV.put(kvKey, data.answer, { expirationTtl: 86400 * 7 });
      const cacheable = json(data, 200);
      await cfCache.put(request, cacheable);
    })());
  }
  return json(data);
}
 ...
 * Manually fire the Supabase process-batch function.
 */
async function handleTriggerBatch(request, env) {
  const auth = request.headers.get('Authorization') || '';
  ...
  const resp = await supabaseFetch(
    '/process-batch',
    { method: 'POST', body: JSON.stringify({ trigger_reason: 'manual_worker' }) },
    env, 'service',
  );
  if (!resp.ok) return err('Batch function error: ' + resp.status, 502);
  return json({ success: true, batch_result: await resp.json() });
}
